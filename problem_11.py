# -*- coding: utf-8 -*-
"""
Created on Fri Jan  1 12:51:36 2016
...
m = [l1,l2,l3,l4,l5,l6,l7,l8,l9,l10,l11,l12,l13,l14,l15,l16,l17,l18,l19,l20]

products=[]
for j in range(0,len(m)):
        for i in range(0,len(m[j])-4):
        #case 1: if position is greater than 4 spots from horizontal edge
        #           can calculate horizontal product
            hor_prod = m[j][i] * m[j][i+1]*m[j][i+2]*m[j][i+3]
            products.append(hor_prod)

for j in range(0,len(m)-4):
        for i in range(0,len(m[j])-4):    
        #case 2: diagonal product
            diag_prod = m[j][i]*m[j+1][i+1]*m[j+2][i+2]*m[j+3][i+3]
            products.append(diag_prod)
            
for j in range(0,len(m)-4):
        for i in range(0,len(m[j])):    
        #case 2: diagonal product
            vert_prod = m[j][i]*m[j+1][i]*m[j+2][i]*m[j+3][i]
            products.append(vert_prod)
            
for j in range(len(m)-1,3,-1):
        for i in range(0,len(m[j])-4):    
        #case 2: diagonal product
            diag_prod2 = m[j][i]*m[j-1][i+1]*m[j-2][i+2]*m[j-3][i+3]
            products.append(diag_prod2)           
